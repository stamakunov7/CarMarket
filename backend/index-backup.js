...
const { initializeRailwayRedis } = require('./redis-config');
initializeRailwayRedis();
...
app.get('/health', async (req, res) => {
  try {
    ...
    const { getCacheStats } = require('./redis-config');
...
app.listen(PORT, () => {
  ...
  logger.info(`CarMarket server started on port ${PORT}`);
});
